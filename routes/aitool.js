...
router.post('/save', authenticateToken, authorizeRoles('superAdmin', 'user'), async (req, res) => {
    try {
...
router.get('/', authenticateToken, authorizeRoles('superAdmin', 'user'), async (req, res) => {
    try {
...
router.get('/getbyId/:id', authenticateToken, authorizeRoles('superAdmin', 'user'), async (req, res) => {
    try {
...
router.delete('/delete/:id', authenticateToken, authorizeRoles('superAdmin'), async (req, res) => {
    try {
...
router.get('/search', authenticateToken, authorizeRoles('superAdmin', 'user'), async (req, res) => {
...
router.get('/filter', authenticateToken, authorizeRoles('superAdmin', 'user'), async (req, res) => {
    ...
    try {
        const filteredTools = await AITool.find({ category: { $regex: category, $options: 'i' } });

        if (filteredTools.length === 0) {
            return res.status(404).json({ message: 'No tools found for this category.' });
        }

        res.status(200).json(filteredTools);
    } catch (error) {
        res.status(500).json({ message: "Error fetching filtered tools", error: error.message });
    }
});
