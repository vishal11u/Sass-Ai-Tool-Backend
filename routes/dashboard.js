...
router.get('/login-signup/:timeframe', async (req, res) => {
    ...
    try {
        const signupCount = await User.countDocuments({ signupDate: { $gte: startDate } });
        const loginCount = await User.countDocuments({ lastLoginDate: { $gte: startDate } });
        
        res.json({ signupCount, loginCount });
    } catch (error) {
        res.status(500).json({ message: 'Error fetching login/signup data', error });
    }
});
...
router.get('/ai-tools/:timeframe', async (req, res) => {
    ...
    try {
        const aiToolCount = await AITool.countDocuments({ createdAt: { $gte: startDate } });
